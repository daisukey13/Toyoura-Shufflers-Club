...
type NoticeRowLoose = {
  ...
  updated_at?: string | null; // ✅ DB追加したのでここで使う
};
...
const fmt = (iso?: string | null) => {
  ...
  if (!s) return '—';
  // 例: "2026-01-28T08:58:10.123Z" を見やすく
  // （タイムゾーン変換は最小に留め、文字列表示に寄せる）
...
export default function AdminNoticeEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const redirectTo = noticeId ? `/admin/notices/${noticeId}/edit` : '/admin/notices';
  ...
  const loadNotice = async () => {
    ...
    try {
      const { data, error: qErr } = await fromAny('notices').select('*').eq('id', noticeId).maybeSingle();
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      // ✅ 更新可能列だけ（updated_at は DB トリガーに任せる）
      const payload = {
      ...
      setMessage('保存しました');
      await loadNotice(); // ← updated_at が更新されて返ってくる
    } catch (e2: any) {
    ...
    <div className="min-h-screen bg-[#2a2a3e] text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} className="bg-gray-900/60 backdrop-blur-md rounded-2xl border border-purple-500/30 p-4 md:p-6 space-y-4">
            ...
            <div className="text-[11px] text-gray-400 space-y-1">
              <div>作成: {fmt(notice.created_at)}</div>
              <div>更新: {fmt(notice.updated_at)}</div>
              <div>公開: {isPublished ? 'ON' : 'OFF'}</div>
            </div>
          </form>
        )}
      </div>

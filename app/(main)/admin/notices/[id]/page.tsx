...
export default function EditNoticePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .eq('id', id)
        .maybeSingle();
      if (error || !data) { router.push('/admin/notices'); return; }
      setTitle(data.title);
      setBody(data.body);
  ...
  if (!isAdmin) return <div className="p-6 text-red-300">権限がありません。</div>;
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="space-y-4 max-w-3xl">
        <div>
          ...
          <input className="w-full rounded border border-purple-500/30 bg-gray-800/50 px-3 py-2 text-white"
                 value={title} onChange={(e)=>setTitle(e.target.value)} />
        </div>

        <div>
          <label className="block text-sm text-gray-300 mb-1">本文（Markdown可）</label>
          <textarea className="w-full rounded border border-purple-500/30 bg-gray-800/50 px-3 py-2 text-white min-h-[220px]"
                    value={body} onChange={(e)=>setBody(e.target.value)} />
        </div>

        <div className="flex gap-4">
          <div>
            <label className="block text-sm text-gray-300 mb-1">ステータス</label>
            <select

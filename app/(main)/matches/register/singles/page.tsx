...
export default function SinglesRegisterPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (alive)
        setMe({ id: user.id, is_admin: !rowErr && Boolean(row?.is_admin) });
    })();
    return () => {
      alive = false;
    };
  ...
  const nameById = (id: string) =>
    (players as Player[]).find((p) => p.id === id)?.handle_name ||
    `${id?.slice(0, 8)}…`;
    ...
    try {
      ...
      if (!res.ok) {
        try {
          ...
          throw new Error(
            j?.message || `登録に失敗しました (HTTP ${res.status})`,
          );
        } catch {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      {playersError && (
        <div className="glass-card rounded-md p-3 mb-4 border border-red-500/40 bg-red-500/10">
          <p className="text-red-300 text-sm">
            プレイヤー一覧の取得に失敗しました。時間をおいて再度お試しください。
          </p>
        </div>
      )}
      {error && (
        <div
          className="glass-card rounded-md p-3 mb-4 border border-red-500/40 bg-red-500/10"
          aria-live="polite"
        >
      ...
      {success && result && (
        <div
          className="glass-card rounded-md p-4 mb-6 border border-green-500/40 bg-green-500/10"
          aria-live="polite"
        >
          ...
          <div className="text-sm text-green-100/90">
            <div className="mb-1">
              ...
              <span className="font-semibold text-green-300">
                {nameById(result.winner_id)}
              </span>{" "}
              ／ 敗者{" "}
              <span className="font-semibold text-red-300">
                {nameById(result.loser_id)}
              </span>
            </div>
            {result.deltas ? (
              <>
                <div className="mt-2">
                  ...
                  <span className="ml-1">
                    勝者 <b>{fmtSigned(result.deltas.winner.points)}</b>
                  </span>
                  <span className="ml-3">
                    敗者 <b>{fmtSigned(result.deltas.loser.points)}</b>
                  </span>
                </div>
                <div className="mt-1">
                  <span className="opacity-80">ハンディキャップ：</span>
                  <span className="ml-1">
                    勝者 <b>{fmtSigned(result.deltas.winner.handicap)}</b>
                  </span>
                  <span className="ml-3">
                    敗者 <b>{fmtSigned(result.deltas.loser.handicap)}</b>
                  </span>
                </div>

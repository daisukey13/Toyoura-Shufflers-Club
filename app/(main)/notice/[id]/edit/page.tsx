// app/admin/notices/[id]/edit/page.tsx など、実際のパスに合わせて配置してください
'use client';
...
export default function EditNoticePage() {
  ...
  const fetchNotice = useCallback(async () => {
    try {
      ...
      if (!data || data.length === 0) throw new Error('Notice not found');

      const noticeData: Notice = data[0];
  ...
  // 権限チェック＋取得（依存に user/isAdmin/fetchNotice）
  const guardAndFetch = useCallback(() => {
    // 未ログイン
    if (!user) {
    ...
    // 取得
    fetchNotice();
  }, [user, isAdmin, router, fetchNotice]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/notices?id=eq.${noticeId}`, {
        method: 'PATCH',
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,

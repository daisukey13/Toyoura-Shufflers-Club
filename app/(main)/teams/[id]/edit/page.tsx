...
export default function TeamEditPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      // チーム本体
      const teamRows = await restGet<Team[]>(
        `/rest/v1/teams?id=eq.${teamId}&select=*`,
        token,
      );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8 text-white">
        <div className="text-center">
          ...
          <Link
            href={`/login?redirect=/teams/${teamId}/edit`}
  ...
  const onAddMember = async (playerId: string) => {
    ...
    try {
      ...
      setMembers((cur) => [
        ...cur,
        { team_id: teamId, player_id: pid, player },
      ]);
  ...
  const onSaveRole = async (playerId: string) => {
    ...
    try {
      ...
      setMembers((cur) =>
        cur.map((m) =>
          m.player_id === playerId ? { ...m, role: role || null } : m,
        ),
      );
    } catch (e: any) {
            ...
            {tab === "members" && (
              <div className="space-y-6">
                ...
                <div className="glass-card rounded-2xl p-4 border border-blue-500/30 bg-blue-900/10 text-sm text-blue-300">
                  ...
                  <b>プレーヤーは1つのチームのみに所属</b>
                  できます（他チーム所属者は追加できません）。
                </div>

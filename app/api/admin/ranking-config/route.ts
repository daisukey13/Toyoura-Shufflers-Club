...
export async function GET() {
  try {
    const { data, error } = await supabaseAdmin
      .from('ranking_config')
      .select('k_factor, score_diff_multiplier, handicap_diff_multiplier, win_threshold_handicap_change, handicap_change_amount')
      .eq('id', 'global')
      .maybeSingle();
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const payload = {
      ...
      score_diff_multiplier: Number(body?.score_diff_multiplier) || DEFAULTS.score_diff_multiplier,
      handicap_diff_multiplier: Number(body?.handicap_diff_multiplier) || DEFAULTS.handicap_diff_multiplier,
      win_threshold_handicap_change: Number(body?.win_threshold_handicap_change) || DEFAULTS.win_threshold_handicap_change,
      handicap_change_amount: Number(body?.handicap_change_amount) || DEFAULTS.handicap_change_amount,

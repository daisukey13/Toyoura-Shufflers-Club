...
async function verifyTurnstile(token: string, remoteip?: string | null) {
  if (!token)
    return { ok: false, codes: ["missing-input-response"] as string[] };
  ...
  try {
    ...
    if (!v.ok) {
      return NextResponse.json(
        { error: "captcha_failed", codes: v.codes },
        { status: 400 },
      );
    }
    ...
    const candidates = Array.from(
      new Set([e164, rawDigits, zeroLeading].filter(Boolean)),
    ) as string[];
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    endpoint: "POST /api/login/resolve-email",
  });
}

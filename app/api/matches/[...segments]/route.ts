...
// ─────────────────────────────────────────────
//  /api/matches/* のキャッチオール
//   - /api/matches/:matchId/report だけ処理対象にする
// ─────────────────────────────────────────────
export async function POST(
  req: NextRequest,
  ctx: { params: { segments: string[] } }
) {
  ...
  if (!matchId || action !== 'report') {
    return NextResponse.json(
      { ok: false, message: 'Invalid matches endpoint.' },
      { status: 404 }
    );
  }
  ...
  try {
    ...
    console.error('[api/matches/[...segments]] fatal:', e);
    return NextResponse.json(
      {
        ok: false,
...
export async function GET(
  ...
  return NextResponse.json({
    ...
    note: 'matches catch-all route is alive',
  });
}

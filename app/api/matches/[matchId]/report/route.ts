...
async function safeUpdateMatches(matchId: string, patch: AnyBody) {
  if (!supabaseAdmin) {
    return { ok: false as const, message: 'Supabase admin client not configured' };
  }
...
export async function POST(
  ...
  try {
    ...
    // ✅ @supabase/ssr が期待する cookie I/F（getAll / setAll）
    const supa = createServerClient(
      ...
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
      {
        cookies: {
          getAll() {
            // cookieStore.getAll() が無い環境でも落とさない
            const all = (cookieStore as any).getAll?.() ?? [];
            return all.map((c: any) => ({ name: c.name, value: c.value }));
          },
          setAll(cookiesToSet: any[]) {
            // Route Handler の cookies() は環境差があるので安全に
            for (const c of cookiesToSet) {
              try {
                (cookieStore as any).set?.(c);
              } catch {
                // 何もしない（最小・安全）
              }
            }
    ...
    if (!aId || !bId) {
      return NextResponse.json(
        {
          ok: false,
          message: 'match の player_a_id / player_b_id が未設定です。',
        },
        { status: 400 }
      );
    }

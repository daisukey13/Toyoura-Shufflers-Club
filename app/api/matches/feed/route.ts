  ...
  try {
    ...
    if (!url || !anon) {
      return NextResponse.json(
        { ok: false, message: "Supabase env missing" },
        { status: 500 },
      );
    }
    ...
    try {
      const { data: mps, error: mpErr } = await client
        .from("match_players")
        .select(
          "match_id, player_id, side_no, rp_delta, hc_delta, ranking_points_delta, handicap_delta",
        )
        .in("match_id", mids);
    ...
    for (const r of mpRows) {
      if (r.match_id && r.player_id)
        mpByMatchPlayer.set(`${r.match_id}:${r.player_id}`, r);
      if (r.match_id && r.side_no != null)
        mpByMatchSide.set(`${r.match_id}:${r.side_no}`, r);
    }
    ...
    const items = (matches ?? []).map((m: MatchRow) => {
      ...
      const mpW =
        mpByMatchPlayer.get(`${m.id}:${wid}`) || mpByMatchSide.get(`${m.id}:1`);
      const mpL =
        mpByMatchPlayer.get(`${m.id}:${lid}`) || mpByMatchSide.get(`${m.id}:2`);

      const wRpDelta =
        pickFirstNumber(
          mpW?.rp_delta,
          mpW?.ranking_points_delta,
      ...
      const wAfterRP =
        typeof wP.ranking_points === "number" ? wP.ranking_points : null;
      const lAfterRP =
        typeof lP.ranking_points === "number" ? lP.ranking_points : null;
      ...
      return {
        ...
        score: {
          winner: m.winner_score ?? 15,
          loser: m.loser_score ?? 0,
          diff: (m.winner_score ?? 15) - (m.loser_score ?? 0),
        },

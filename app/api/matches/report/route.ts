...
// ─────────────────────────────────────────────────────────────
// ★今回の最小修正：matchId を query だけでなく path からも拾う
// ─────────────────────────────────────────────────────────────
function extractMatchIdFromPath(req: NextRequest) {
  const path = String(req.nextUrl?.pathname ?? '');
  // /api/matches/<uuid>/report  or /api/matches/<uuid>/report/
  const m = path.match(/^\/api\/matches\/([^/]+)\/report\/?$/);
  return m?.[1] ? String(m[1]).trim() : '';
}

function getMatchId(req: NextRequest) {
  const q = String(req.nextUrl.searchParams.get('matchId') ?? '').trim();
  if (q) return q;

  const p = extractMatchIdFromPath(req);
  if (p) return p;
...
export async function GET(req: NextRequest) {
  ...
  if (!matchId) {
    return NextResponse.json(
      { ok: false, message: 'matchId がありません（query か path に matchId が必要です）' },
      { status: 400 },
    );
  }
  return NextResponse.json({ ok: true, route: '/api/matches/report', matchId }, { status: 200 });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[api/matches/report] fatal:', e);

...
async function verifyTurnstile(req: Request, token?: string) {
  ...
  try {
    ...
    return {
      ...
      codes: j?.["error-codes"] as string[] | undefined,
    };
  } catch {
  ...
  try {
        ...
        tv.reason === "missing-secret"
          ...
          : tv.reason === "missing-token"
            ...
            : tv.reason === "network-error"
              ...
              : "Turnstile 検証に失敗しました。";
      return NextResponse.json(
        { ok: false, message: msg, codes: (tv as any).codes },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        {
          ok: false,
          message: "サーバ設定不備: Supabase 環境変数が未設定です。",
        },
        { status: 500 },
      );
    }
    ...
    if (error) {
      ...
      console.error("[auth/phone] signInWithOtp error:", {
        status,
      ...
      return NextResponse.json(
        {
          ...
          message:
            status === 429
              ...
              : "SMS の送信に失敗しました。時間をおいて再試行してください。",
        },

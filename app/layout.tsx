...
import { FaHome, FaTrophy, FaUsers, FaFlagCheckered, FaIdBadge, FaUserCircle } from 'react-icons/fa';
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  cookies: {
    ...
    set(name: string, value: string, options: CookieOptions) {
      try {
        cookieStore.set({ name, value, ...(options || {}), path: (options as any)?.path ?? '/' } as any);
      } catch {
    ...
    remove(name: string, options: CookieOptions) {
      try {
        cookieStore.set({ name, value: '', ...(options || {}), path: (options as any)?.path ?? '/', maxAge: 0 } as any);
      } catch {
        /* no-op on server component render */
      }
    },

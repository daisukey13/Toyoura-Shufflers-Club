...
export default function TurnstileWidget({
  ...
  useEffect(() => {
    const key = siteKey ?? (process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY as unknown as string);
    if (typeof key !== 'string' || !key) {
      console.error('[Turnstile] invalid siteKey. Set NEXT_PUBLIC_TURNSTILE_SITE_KEY.');
      return;
    }

    if (!window.__turnstileScriptPromise) {
      window.__turnstileScriptPromise = new Promise<void>((resolve) => {
        const existed = document.querySelector<HTMLScriptElement>(`script[src^="${SCRIPT_SRC}"]`);
        if (existed) {

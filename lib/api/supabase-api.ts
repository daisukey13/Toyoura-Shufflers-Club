...
export class SupabaseAPI {
  ...
  static async createPlayer(data: any): Promise<ApiResponse<any>> {
    ...
    const response = await this.fetchWithRetry<any[]>("/rest/v1/players", {
      method: "POST",
    ...
    return {
      data: null,
      error: response.error || "Failed to create player",
      status: 400,
    };
  }
  ...
  static async createMatch(matchData: any): Promise<ApiResponse<any>> {
    const response = await this.fetchWithRetry<any>("/rest/v1/matches", {
      method: "POST",

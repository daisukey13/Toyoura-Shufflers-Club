...
/**
 * 目的：
 * - Service Role を使う supabaseAdmin を維持（機能は変えない）
 * - “Invalid API key” 切り分けのための検査を維持（キー全文は出さない）
 *
 * NOTE:
 * - ここはサーバ専用（Service Role / Secret key）
 * - クライアント anon/publishable とは別物
 */

// ✅ URL は NEXT_PUBLIC_* でも動くが、サーバ用変数も許容しておく（互換 & 安全）
const SUPABASE_URL = (
...
// 未設定なら即失敗（既存方針維持）
if (!SUPABASE_URL) {
  throw new Error('[supabaseAdmin] Missing env: SUPABASE_URL (or NEXT_PUBLIC_SUPABASE_URL)');
}
if (!SERVICE_ROLE_KEY) {
  throw new Error('[supabaseAdmin] Missing env: SUPABASE_SERVICE_ROLE_KEY');
}

// 誤ってブラウザで動いたら止める（既存方針維持）
...
function b64urlDecode(input: string): string {
  ...
  // ✅ Edge などで Buffer が無い可能性に備える（落とさない）
...
function tryAssertProjectMatch(url: string, key: string) {
  ...
  if (parts.length === 3) {
    try {
      ...
      // ✅ 解析失敗でも落とさずヒントだけ出す（既存方針維持）
      console.warn('[supabaseAdmin] JWT inspection failed:', e?.message ?? e);
    }
    return;
  }

  // ② sb_secret_ 等 JWTじゃない場合
  if (/\s/.test(key)) {
...
// ログ（キー全文は出さない）
// ✅ 本番でうるさければ SUPABASE_ADMIN_DEBUG=1 の時だけ出す運用も可
const shouldLog = process.env.NODE_ENV !== 'production' || process.env.SUPABASE_ADMIN_DEBUG === '1';
if (shouldLog) {
